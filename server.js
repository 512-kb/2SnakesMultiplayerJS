...
server.listen(3000,() => {
  generateFood();
  snakes.push({  
      headColor: '#3dff47' , bodyColor: 'green' , 
      direction: "right" ,   prevDirection: "right" , 
      x : 5*grid , y : 5*grid, 
      cells: [{x: 5*grid, y: 5*grid} , {x: 5*grid - grid  , y: 5*grid}] 
    });
    snakes.push({  
       headColor: '#ff36a8' , bodyColor: '#75008f' , 
       direction: "up" ,      prevDirection: "up" , 
       x : canvas.width-5*grid , y : canvas.height-5*grid, 
       cells: [{x: canvas.width-5*grid, y: canvas.height-5*grid} , {x: canvas.width-5*grid  , y: canvas.height-5*grid + grid}] 
    })
  })
...
io.on('connection',function(socket){
  ...
  socket.on('gameStart',() => {
    if(gameOver) { gameOver = false; autoMove() }
    gameOver = false;    
  }) 
  socket.on('gameOver',() => {
    stopGame();
...
function moveSnake(index)
{
  ...
  {
    case "left":   dx = -grid;  break;
    case "up":     dy = -grid;  break;
    case "right":  dx = grid;   break;
    case "down":   dy = grid;   break;
  }
